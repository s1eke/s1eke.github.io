...
```
  ...
  └───────────┬───────────┘
              ...
              ▼
  ┌───────────────────────┐
  │  Windows NT 内核       │ ← 识别为 Pico 进程的系统调用
  │  系统服务分发器        │
  └───────────┬───────────┘
              │ 转发给 Pico 提供程序
              ▼
  ┌───────────────────────┐
  │  lxcore.sys            │ ← Pico 提供程序驱动
  │  (Linux 系统调用翻译)  │
  └───────────┬───────────┘
              │
    ┌─────────┴─────────┐
...
```mermaid
timeline
    title Windows 平台 Unix/Linux 兼容性技术演进
    1993 : POSIX 子系统
         : 最低限度合规
         : Windows NT 3.1
    1999 : SFU / Interix
